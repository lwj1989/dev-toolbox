<template>
  <div class="relative group" ref="switcherRef">
    <button class="p-2 rounded-lg hover:bg-secondary transition-colors btn-icon" title="切换工具">
    ...
    <div class="absolute top-full left-0 mt-0.5 w-48 bg-card border border-border rounded-lg shadow-lg z-50 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
      <div class="p-2">
        <router-link 
          ...
          :key="module.id" 
          :to="module.route" 
          class="flex items-center space-x-2 px-3 py-2 rounded-md text-sm text-foreground hover:bg-secondary transition-all duration-150 active:bg-secondary/70 active:scale-95"
        >
